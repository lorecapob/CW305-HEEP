...
typedef union {
  ...
  volatile uint32_t w[5][2];
  volatile uint8_t b[5][8];
} ascon_state_t;
...
typedef union {
  volatile uint64_t x[ASCON_KEYWORDS];
  volatile uint32_t w[ASCON_KEYWORDS][2];
  volatile uint8_t b[ASCON_KEYWORDS][8];
} ascon_key_t;
